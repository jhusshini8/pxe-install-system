...
def GetInstallList(request):
    if (request.method == 'POST'):
        postBody = request.body
        print(postBody)

    server = 'http://192.168.8.101/cobbler_api'
    user = 'cobbler'
    password = 'cobbler'
    ...
    try:
        ...
        # eval(postBody).get("nodehostname")
        print remote_server.get_systems()
        # str_json = json.dumps(remote_server.get_systems())
        # print str_json
        ...
        if remote_server.ping():  # cobbler服务器状态监测
            if eval(postBody).get("nodehostname") in remote_server.find_system():
                ...
                remote_server.modify_system(system_id, "name", "web3", token)
                remote_server.modify_system(system_id, "hostname", "web3", token)
                remote_server.modify_system(system_id, 'modify_interface', {
                    "macaddress-eth0": "00:0C:29:e1:8a:5f",
                    "ipaddress-eth0": "192.168.100.154",
                    "Gateway-eth0": "192.168.100.4",
                    "subnet-eth0": "255.255.255.0",
                    "static-eth0": 1,
                    "dnsname-eth0": "192.168.100.6"
                }, token)
                remote_server.modify_system(system_id, "profile", "centos7.8", token)
