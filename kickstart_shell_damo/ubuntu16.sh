...
#update-grub
apt update -y
apt-get install -y ssh vim curl
...
hostip=`ifconfig  eno1 | grep "inet addr" | awk '{print $2}' | awk -F':' '{print $2}'`
hostmask=`ifconfig  eno1 | grep "inet addr" | awk '{print $4}' | awk -F':' '{print $2}'`
...
sed -i '/ubuntu16/d' /etc/rc.local
rm -rf /etc/default/ubuntu16.sh
